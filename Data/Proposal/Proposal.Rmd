...
**Division of labor:**
| Name | Initial work assignment | 
| ------------- | ------------- |
| Annie Cavalla | Section 3: TF binding site enrichment |
| Rawnak Hoque  | Section 2: RNA-seq analysis of differentially expressed genes  
| Somdeb Paul | Section 4: TF pattern function analysis
| Fangwu Wang  | Section 1: DNA methylation analysis |

All members will also have input into the poster on their area of expertise.


**Datasets:**
We will work with DNA methylation (bisulfite-seq) and RNA-seq data from the published BluePrint Epigenome project [6] which were downloaded from GEO (GSE 87197). All samples were collected from primary human peripheral blood cells in seven stem/progenitor populations: HSC, MPP, CMP, CLP, GMP, MEP, MLP. The DNA methylation data is alignment raw data at single base resolution and quality control of sequencing is available at RnBead website [9]. For each population, we chose one sample with multiple pools (technical replicates) based on donor age and data quality. The corresponding RNA-seq data from same populations (same tissue origin but not same donors) was obtained from GEO (GSE 87195). The data has been processed with differential expression analysis on transcript level and we will further quantify it on the gene level.
Since the BluePrint data does not have sample replicates within cell types, we will leverage the results of our TF enrichment and gene expression analysis on RNA-seq data from firebrowse.org to cluster human leukaemias into subtypes by PCA. 



**Aims and Methods:**

1.Identify active DNA methylation regions and associate with genes: use RnBeads package to sum up the reads from all replicates and estimate methylation values (see suppl methods of the paper); quality control; differential methylation analysis with RnBeads; define (set threshold) and identify hypomethylated regions for each population; map regulatory regions (promoter, enhancer) to the hypomethylated regions; identify genes associated with the regulatory regions.
